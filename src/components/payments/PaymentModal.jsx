...
function createWhatsAppMessage(student, bills) {
  ...
  const jumlahBelumLunas = sortedBills.filter(
    (bill) => bill.status !== "Lunas",
  ).length;

  const detailTagihan = sortedBills
    .map((bill, index) => {
      const status = bill.status === "Lunas" ? "LUNAS" : "BELUM LUNAS";

      return [
        `${index + 1}. ${bill.nama || "Tagihan"}`,
        `   Nominal     : ${formatRupiah(bill.nominal)}`,
        `   Terbayar    : ${formatRupiah(bill.dibayar)}`,
        `   Potongan    : ${formatRupiah(bill.potongan)}`,
  ...
  return [
    ...
    `Assalamu'alaikum,`,
    ``,
    `Berikut informasi tagihan pembayaran siswa:`,
    ``,
    `*Nama : ${student.nama}*`,
    `NIS  : ${student.nis}`,
    ...
    ``,
    `Tagihan Lunas : ${jumlahLunas}`,
    `Belum Lunas   : ${jumlahBelumLunas}`,
    ``,
    totalSisa > 0
      ? `Mohon diperhatikan, masih terdapat sisa tagihan sebesar *${formatRupiah(totalSisa)}*.`
      : `Alhamdulillah, seluruh tagihan siswa sudah *LUNAS*.`,
    ``,
    `Demikian informasi tagihan siswa.`,
    `Terima kasih.`,
  ].join("\n");
}
...
export default function PaymentModal({ open, student, onClose }) {
  ...
  function handleShareWhatsApp() {
    ...
    const message = createWhatsAppMessage(student, bills);
        ...
        <div className="sticky top-0 z-20 relative border-b border-zinc-100 dark:border-white/10 bg-white/80 dark:bg-[#131316]/80 backdrop-blur-xl px-7 py-5 shrink-0">
          ...
          <div className="flex items-start justify-between gap-4 pr-10">
            ...
            <div className="min-w-0">
              ...
              <h2 className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-white truncate">
                {student.nama}
              </h2>
            ...
            <button
              ...
              title="Bagikan detail tagihan melalui WhatsApp"
              className="
              shrink-0
              inline-flex
              items-center
